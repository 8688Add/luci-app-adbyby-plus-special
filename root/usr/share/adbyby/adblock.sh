#!/bin/sh
	...
	if [ "$(uci -q get adbyby.@adbyby[0].wan_mode)" == "1" ];then
		...
		if [ -s "/tmp/dnsmasq.adblock" ];then
			...
			if ( ! cmp -s /tmp/dnsmasq.adblock /usr/share/adbyby/adbyby_adblock/dnsmasq.adblock );then
				cp -f /tmp/dnsmasq.adblock /usr/share/adbyby/adbyby_adblock/dnsmasq.adblock
