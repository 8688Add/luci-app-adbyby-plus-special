#!/bin/sh
	...
	while : ; do
		wget -T 3 -O /usr/share/adbyby/adbyby https://cdn.jsdelivr.net/gh/small-5/adbyby/$ARCH
		if [ "$?" == "0" ];then
	...
	while : ; do
		wget -T 3 -O /usr/share/adbyby/md5 https://cdn.jsdelivr.net/gh/small-5/adbyby/md5
		if [ "$?" == "0" ];then
			break
		else
			sleep 2
	...
	if [ "$(uci -q get adbyby.@adbyby[0].wan_mode)" == "1" ];then
		...
		while : ; do
			wget -T 3 -O /usr/share/adbyby/adbyby_adblock/dnsmasq.adblock https://cdn.jsdelivr.net/gh/small-5/adbyby/dnsmasq.adblock
			[ "$?" == "0" ] && break || sleep 2
		done

		while : ; do
			wget -T 3 -O /usr/share/adbyby/md5 https://cdn.jsdelivr.net/gh/small-5/adbyby/md5_1
			if [ "$?" == "0" ];then
				break
			else
				sleep 2
