#!/bin/sh
	...
	wget -4 -t 9 -T 3 -O /usr/share/adbyby/adbyby https://dev.tencent.com/u/Small_5/p/adbyby/git/raw/master/$ARCH
	if [ "$?" == "0" ]; then
	...
	echo "开始下载MD5文件..."
	wget -4 -t 9 -T 3 -O /usr/share/adbyby/md5 https://dev.tencent.com/u/Small_5/p/adbyby/git/raw/master/md5
	if [ "$?" != "0" ]; then
		echo "下载MD5文件失败，请重试！"
	...
	if [ "$(uci -q get adbyby.@adbyby[0].wan_mode)" == "1" ]; then
		...
		wget -4 -t 9 -T 3 -O /usr/share/adbyby/adbyby_adblock/dnsmasq.adblock https://dev.tencent.com/u/Small_5/p/adbyby/git/raw/master/dnsmasq.adblock
		if [ "$?" != "0" ];then
		...
		wget -4 -t 9 -T 3 -O /usr/share/adbyby/md5 https://dev.tencent.com/u/Small_5/p/adbyby/git/raw/master/md5_1
		if [ "$?" != "0" ]; then
